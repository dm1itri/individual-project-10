...
def game(game_id):
    ...
    if not request.cookies.get('number_move') or request.cookies.get('game_id') != str(game_id):
        ...
        with db_session.create_session() as session:
            player = session.query(Player).filter(Player.is_occupied == False,
                                                  Player.game_id == game_id).order_by(
                Player.number_move).first()
            if not player:
...
def create_game():
    ...
    with db_session.create_session() as session:
        game_id = add_game(session, int(request.form.get('number_of_players')))
        add_players(session, game_id, int(request.form.get('number_of_players')))
